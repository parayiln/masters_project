...
class LeaderDetector:
    ...
    def __init__(self, image_dict, b_output_debug=True, b_recalc=False):
        ...
        for i, stats in enumerate(self.vertical_leader_stats):
            ...
            if b_output_debug:
                ...
                self.bezier_mpt = quad.draw_quad(im_orig_debug, (0,0,255))
                if quad.is_wire():
                ...
                quad.draw_quad(self.images["RGB_Stats"], (0,0,0))
    ...
    def fit_quad(self, im_mask, pts, b_output_debug=True, quad_name=0):
        ...
        if b_output_debug:
            ...
            quad.draw_quad(im_debug, (255,255,255))
    ...
    def adjust_quad_by_flow(self):
        ...
        for i, quad in enumerate(self.vertical_leader_quads):
            ...
            if b_output_debug:
                ...
                # Draw the original, the edges, and the depth mask with the fitted quad
                quad_flow.draw_quad(im_orig_debug, (255,0,0))
                ...
                quad_flow.draw_quad(self.images["RGB_Stats"], (255,0,0))
    ...
    def fit_quad_flow(self, im_flow_mask, quad, b_output_debug=True):
        ...
        if b_output_debug:
            ...
            quad.draw_quad(im_debug,(255,0,0))
            quad.draw_boundary(im_debug)
