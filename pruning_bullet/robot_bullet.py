...
class Robot(): 
    def __init__(self):
        ...
        self.robotID = p.loadURDF(self.robotUrdfPath, self.robotStartPos, self.robotStartOrn,flags=p.URDF_USE_SELF_COLLISION_EXCLUDE_PARENT)
        ...
        self.joints = dict()
        self.control_joints = ["7thjoint_prismatic", "shoulder_pan_joint", "shoulder_lift_joint", "elbow_joint", "wrist_1_joint", "wrist_2_joint", "wrist_3_joint"]
    ...
    def get_joint_state(self, joint_name):
        ...
        joint_position, joint_vel, joint_force, torque = p.getJointState(self.robotID, joint_id)
    ...
    def move_joints(self, joint_values, control_type):
        ...
        p.setJointMotorControlArray(self.robotID, indexes,controller,targetPositions, targetVelocities, positionGains=[0.05]*len(poses),forces=forces)
    ...
    def move_Onejoint(self, targets, joint_name,  control_type, include_prismatic=True):
        ...
        p.setJointMotorControl2(self.robotID, id,controller,targetPositions, targetVelocities)
           


    def get_links(self):
    
        linkIDs = list(map(lambda linkInfo: linkInfo[1], p.getVisualShapeData(self.robotID)))
        linkNum = len(linkIDs)
        return linkNum
    ...
    def get_link_kinematics(self, link_name_or_id, use_com_frame=False, as_matrix=False):
        ...
        rez = p.getLinkState(self.robotID, link_id, computeForwardKinematics=True)
