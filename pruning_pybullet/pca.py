...
class PCA(object):
    def __init__(self, image):
        self.dir_labeled = r'/home/nidhi/masters_project/plots/'
...
####################################################################
    def getOrientation(self, pts, img2):
        ...
        p1 = (self.curr_target[0] + 0.02 * eigenvectors[0, 0] * eigenvectors[0, 0],
              self.curr_target[1] + 0.02 * eigenvectors[0, 1] * eigenvectors[0, 0])
        p2 = (
...
####################################################################
    def image_process(self, step):
        ...
        cropped_image = self.img[self.cropped_height:self.cropped_height +
                                 self.pixel, 0:self.img.shape[1]]
        bw = self.img_binary[self.cropped_height:self.cropped_height +
                             self.pixel, 0:self.img.shape[1]]
        im2, contours, hera = cv.findContours(
...
if __name__ == "__main__":
    for k in range(0, 3):
        ...
        for j in range(3, len(pca.vector_list)):
            if flag == 0:
                v1 = np.array([pca.vector_list[j-2][0],
                              pca.vector_list[j-2][1]])
                v_2 = np.array(
                    [pca.vector_list[j-1][0], pca.vector_list[j-1][1]])
                ...
                v0 = v2
                # Go along the edge at 1.5 * the estimated radius
                step_size_to_use = int(quad.radius_2d * 1.5)
